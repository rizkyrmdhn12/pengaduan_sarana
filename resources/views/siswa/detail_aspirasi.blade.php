    ...
    <div class="card card-smk mb-4">
        <div class="card-body py-4">
            ...
            <div class="progress-tracker">
                ...
                <div class="tracker-step {{ $aspirasi->status=='Menunggu' ? 'current' : 'done' }}">
                    <div class="tracker-icon">⏳</div>
                    <div class="small fw-semibold">Menunggu</div>
                    <div class="text-muted" style="font-size:.72rem">Antri ditinjau</div>
                </div>
                <div class="tracker-step {{ $aspirasi->status=='Proses' ? 'current' : ($aspirasi->status=='Selesai' ? 'done' : '') }}">
                ...
                <div class="tracker-step {{ $aspirasi->status=='Selesai' ? 'done' : '' }}">
                    <div class="tracker-icon">✅</div>
                ...
                <div class="card-body">
                    <table class="table table-borderless small">
                        ...
                        <tr>
                            ...
                            <td><span class="badge {{ $aspirasi->jenis=='sarana_prasarana' ? 'badge-sarana' : 'badge-kesejahteraan' }}">{{ $aspirasi->ket_kategori }}</span></td>
                        </tr>
                        <tr><td class="text-muted fw-semibold">Lokasi</td><td class="fw-semibold">{{ $aspirasi->lokasi }}</td></tr>
                        <tr><td class="text-muted fw-semibold">Tanggal</td><td>{{ \Carbon\Carbon::parse($aspirasi->created_at)->format('d M Y, H:i') }}</td></tr>
                        <tr>
                            <td class="text-muted fw-semibold">Status</td>
                            <td>
                                @if($aspirasi->status=='Menunggu') <span class="badge badge-menunggu fs-6">⏳ {{ $aspirasi->status }}</span>
                                @elseif($aspirasi->status=='Proses') <span class="badge badge-proses fs-6">⚙️ {{ $aspirasi->status }}</span>
                                @else <span class="badge badge-selesai fs-6">✅ {{ $aspirasi->status }}</span>
                                @endif
                            </td>
                        </tr>
                        @if($aspirasi->anonim)
                    ...
                    <div class="p-3 rounded-3 small mb-3" style="background:#f8f9fa;border-left:4px solid var(--smk-biru)">{{ $aspirasi->ket }}</div>

                    @if($aspirasi->foto)
                    <p class="text-muted fw-semibold small mb-1"><i class="bi bi-image me-1"></i>Foto Bukti:</p>
                    @php $fotoUrl = \Illuminate\Support\Facades\Storage::url($aspirasi->foto); @endphp
                    <img src="{{ $fotoUrl }}" class="foto-preview w-100"
                         onclick="showFoto('{{ $fotoUrl }}')"
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="p-3 rounded-3 flex-fill" style="background:rgba(39,174,96,.08);border:2px solid rgba(39,174,96,.3)">
                        <div class="d-flex align-items-center mb-2">
                            ...
                            <span class="text-muted small">{{ \Carbon\Carbon::parse($aspirasi->status_updated_at)->format('d M Y') }}</span>
                        </div>
